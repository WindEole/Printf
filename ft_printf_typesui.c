...
/*   Updated: 2021/03/06 15:07:20 by iderighe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_type_is_unint(unsigned int input, t_flags *flags)
{
	...
	if (flags->dot == 0 && flags->width == 0 && input == 0)
		return (0);
/*	if (flags->dot == 0 && flags->width == 0 && input != 0)
...
/*int		ft_filldotzero_ui(char *str_input, t_flags flags)
{
	int		add;

	add = 0;
	while (flags.dot > (int)ft_strlen(str_input))
	{
		add += ft_putchar('0');
		flags.dot--;
	}
